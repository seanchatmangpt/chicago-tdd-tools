# v1.2.0 Coverage Strategy and Goals (Planned)

## Executive Summary

**Status**: This document describes planned improvements for v1.2.0. Current version is 1.1.2.

Chicago TDD Tools v1.2.0 will introduce **mandatory 85% line coverage** enforcement, up from v1.1.0's warning-only 70% threshold. This document outlines the coverage strategy, goals, and improvement plan for the upcoming release.

## Coverage Metrics

| Metric | v1.1.0 | v1.2.0 Goal | Status |
|--------|--------|------------|--------|
| Minimum line coverage | 70% (warning) | **85%** (enforced) | 📈 +15% |
| Production code target | N/A | 90%+ | New |
| Test utilities target | N/A | 80%+ | New |
| CI/CD enforcement | Warning only | Hard requirement | ✅ Blocking |
...
```yaml
- name: Check coverage threshold (≥85% required)
