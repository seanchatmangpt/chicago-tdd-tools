...
///
/// Requires `ticket` = "path/to/ticket.md" and `test` = "path/to/test.rs" — both must
/// exist at workspace root. Expands to `::chicago_tdd_tools::__runtime::scaffold_pending`(...)
/// — NOT `todo!()` — so hollow detectors do not flag it.
#[proc_macro]
...
/// Renames the test to `chicago_cc_NNN`_&lt;original&gt; and wraps the body in `catch_scaffold()`.
#[proc_macro_attribute]
