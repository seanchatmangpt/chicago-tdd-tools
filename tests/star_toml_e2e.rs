...
chicago_tdd_tools::test!(test_f1_load_multiple_configs, {
    let (code, stdout, _) = run_bin(
        &[
            "--config",
            "examples/star-toml/samples/default.toml",
            "--config",
            "examples/star-toml/samples/dev.toml",
        ],
        &[],
    );
    assert_eq!(code, 0);
...
chicago_tdd_tools::test!(test_f1_parse_nonexistent_file, {
    let (code, _, stderr) =
        run_bin(&["--config", "examples/star-toml/samples/does_not_exist.toml"], &[]);
    assert_ne!(code, 0);
    assert!(
        stderr.contains("FileNotFound") || stderr.contains("not found") || stderr.contains("exist")
    );
});
...
chicago_tdd_tools::test!(test_f2_merge_nested_table, {
    let (code, stdout, _) = run_bin(
        &[
            ...
            "examples/star-toml/samples/default.toml",
            "--config",
            "examples/star-toml/samples/prod.toml",
        ],
        &[],
    );
    assert_eq!(code, 0);
...
chicago_tdd_tools::test!(test_f2_merge_overlapping_keys, {
    let (code, stdout, _) = run_bin(
        &[
            ...
            "examples/star-toml/samples/prod.toml",
            "--config",
            "examples/star-toml/samples/dev.toml", // overrides port to 80
        ],
        &[],
    );
    assert_eq!(code, 0);
...
chicago_tdd_tools::test!(test_f3_validate_port_out_of_range, {
    let (code, _, stderr) =
        run_bin(&["--config", "examples/star-toml/samples/invalid_port.toml"], &[]);
    assert_ne!(code, 0);
    assert!(
...
chicago_tdd_tools::test!(test_f3_validate_workers_out_of_range, {
    let (code, _, stderr) =
        run_bin(&["--config", "examples/star-toml/samples/invalid_workers.toml"], &[]);
    assert_ne!(code, 0);
    assert!(
...
chicago_tdd_tools::test!(test_f3_validate_tls_paths_required, {
    let (code, _, stderr) = run_bin(
        &[
            ...
            "examples/star-toml/samples/default.toml",
            "--config",
            "examples/star-toml/samples/invalid_tls.toml",
        ],
        &[],
    );
    assert_ne!(code, 0);
    assert!(
        stderr.contains("tls_cert_required")
            || stderr.contains("cert_path")
            || stderr.contains("key_path")
            || stderr.contains("Invalid")
    );
});
...
chicago_tdd_tools::test!(test_f4_progress_alert_success, {
    ...
    assert_eq!(code, 0);
    assert!(
        stdout.contains("SUCCESS") || stdout.contains("Admitted") || stdout.contains("admitted")
    );
});
...
chicago_tdd_tools::test!(test_f4_progress_alert_warning_port, {
    ...
    assert!(
        stdout.contains("WARNING")
            || stdout.contains("warning")
            || stdout.contains("prefer a port above 1024")
    );
});
...
chicago_tdd_tools::test!(test_b2_port_lower_boundary_zero, {
    ...
    assert_ne!(code, 0);
    assert!(
        stderr.contains("port") || stderr.contains("out_of_range") || stderr.contains("Invalid")
    );
...
chicago_tdd_tools::test!(test_b2_workers_lower_boundary_zero, {
    ...
    assert_ne!(code, 0);
    assert!(
        stderr.contains("workers") || stderr.contains("out_of_range") || stderr.contains("Invalid")
    );
    let _ = std::fs::remove_file(path);
});
...
chicago_tdd_tools::test!(test_b2_float_instead_of_integer, {
    ...
    assert_ne!(code, 0);
    assert!(
        stderr.contains("Parse") || stderr.contains("invalid type") || stderr.contains("integer")
    );
    let _ = std::fs::remove_file(path);
});
...
chicago_tdd_tools::test!(test_b2_log_level_case_sensitivity, {
    ...
    assert_ne!(code, 0);
    assert!(
        stderr.contains("log_level") || stderr.contains("not_one_of") || stderr.contains("Invalid")
    );
    let _ = std::fs::remove_file(path);
});
...
chicago_tdd_tools::test!(test_b2_mixing_array_and_table, {
    ...
    assert_ne!(code, 0);
    assert!(
        stderr.contains("Parse") || stderr.contains("invalid type") || stderr.contains("Invalid")
    );
    let _ = std::fs::remove_file(path);
});
...
chicago_tdd_tools::test!(test_b2_large_file_parsing, {
    ...
    assert_eq!(code, 0);
    assert!(
        stdout.contains("admitted") || stdout.contains("Admitted") || stdout.contains("SUCCESS")
    );
    let _ = std::fs::remove_file(path);
});
...
chicago_tdd_tools::test!(test_c3_merge_valid_and_invalid, {
    let (code, _, stderr) = run_bin(
        &[
            ...
            "examples/star-toml/samples/default.toml",
            "--config",
            "examples/star-toml/samples/invalid_port.toml",
        ],
        &[],
    );
    assert_ne!(code, 0);
...
chicago_tdd_tools::test!(test_c3_multiple_errors_from_different_layers, {
    let (code, _, stderr) = run_bin(
        &[
            ...
            "examples/star-toml/samples/invalid_port.toml",
            "--config",
            "examples/star-toml/samples/invalid_workers.toml",
        ],
        &[],
    );
    assert_ne!(code, 0);
...
chicago_tdd_tools::test!(test_r4_local_override_flow, {
    ...
    let (code, stdout, _) = run_bin(
        &["--config", "examples/star-toml/samples/default.toml", local_override.to_str().unwrap()],
        &[],
    );
    assert_eq!(code, 0);
...
chicago_tdd_tools::test!(test_r4_ci_environment_strict, {
    ...
    let (code, stdout, _) =
        run_bin(&["--config", "examples/star-toml/samples/default.toml"], &[("CI", "true")]);
    assert_eq!(code, 0);
    assert!(
        stdout.contains("SUCCESS") || stdout.contains("admitted") || stdout.contains("Admitted")
    );
...
chicago_tdd_tools::test!(test_r4_microservice_scaling, {
    ...
    let (code, stdout, _) = run_bin(
        &["--config", "examples/star-toml/samples/default.toml", scale_config.to_str().unwrap()],
        &[],
    );
    assert_eq!(code, 0);
    ...
    SnapshotAssert::with_settings(
        ...
        || {
            SnapshotAssert::assert_matches(
                &actual_toml,
                "star_toml_e2e__star_toml_default_config_snapshot",
            );
        },
    );
}
...
fn test_observability_configuration_load() {
    ...
    attributes
        .insert("config.file".to_string(), "examples/star-toml/samples/default.toml".to_string());
    ...
    let test =
        ObservabilityTest::with_config(config).expect("Failed to initialize ObservabilityTest");
