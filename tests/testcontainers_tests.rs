...
mod testcontainers_tests {
    ...
    #[test]
    fn test_concurrent_container_creation() {
        if skip_if_docker_unavailable() {
            return;
        }
            ...
            .map(|i| {
                ...
                thread::spawn(move || {
                    ...
                    let container_result =
                        GenericContainer::new(client.client(), "alpine", "latest");
    ...
    #[test]
    fn test_stress_concurrent_exec() {
        if skip_if_docker_unavailable() {
            return;
        }
    ...
    #[test]
    fn test_stress_multiple_containers_concurrent_exec() {
        if skip_if_docker_unavailable() {
            return;
        }
